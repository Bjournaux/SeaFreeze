...
class TestLoadGibbsMLSpline(ut.TestCase):
    ...
    def test_loadgibbs_3d_ScalarGo(self):
        ...
        self.assertTrue(np.array_equal(gsp['sp']['MW'], np.array([0.01801528, 0.05844])))
        self.assertEqual(gsp['sp']['nu'], 2)
    def test_loadgibbs_2d(self):
        ...
        self.assertTrue(np.array_equal(gsp['sp']['MW'], np.array([0.01801528])))
        self.assertFalse('nu' in gsp['sp'])

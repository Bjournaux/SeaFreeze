...
%
% Requirements:
%   - SeaFreeze_Gibbs.mat and SeaFreeze_Gibbs_VII_NaCl5GPa.mat must be on
%     the MATLAB path (run from Matlab/). No toolboxes required.
%
...
%   NaN = outside all parametrisation ranges
    ...
    methods (Test)
        ...
        % ================================================================== %
        %  NaCl aqueous mode  (freezing-point depression)
        % ================================================================== %

        function test_NaCl_scatter_liquid_at_268K(testCase)
            % Pure water freezes at ~273 K at 0.1 MPa; 2 mol/kg NaCl
            % depresses the freezing point to ~268 K → should still be liquid.
            out = SF_WhichPhase([0.1 268 2.0], 'solute', 'NaCl');
            testCase.verifyEqual(out, 0, ...
                '2 mol/kg NaCl at 268 K should be liquid (FPD)');
        end

        function test_NaCl_scatter_ice_at_240K(testCase)
            % At 240 K even a 2 mol/kg brine should be frozen.
            out = SF_WhichPhase([0.1 240 2.0], 'solute', 'NaCl');
            testCase.verifyEqual(out, 1, ...
                '2 mol/kg NaCl at 240 K should be ice Ih');
        end

        function test_NaCl_grid_output_shape(testCase)
            % Grid {P, T, m} with scalar m → output shape [nP × nT].
            P = [0.1 50 100];   % 3 pressures
            T = [250 260 270];  % 3 temperatures
            m = 1.0;            % scalar molality
            out = SF_WhichPhase({P, T, m}, 'solute', 'NaCl');
            testCase.verifySize(out, [numel(P) numel(T)], ...
                'NaCl grid output must be [nP × nT] for scalar m');
        end
